...
@TeleOp
public class NormalizedColoursTest extends LinearOpMode {
    private NormalizedColorSensor sensor_color;
    double hue;

    @Override
    public void runOpMode() {
        sensor_color = hardwareMap.get(NormalizedColorSensor.class, "sensor_colour");

        waitForStart();

        while (opModeIsActive()) {
            telemetry.addData("Light Detected", ((OpticalDistanceSensor) sensor_color).getLightDetected());
